...
exports.AllowedOperationNames = AllowedOperationNames;
...
function AllowedOperationNames(context, options) {
  var allowedOperationNames = options.allowedOperationNames;


  if (!Array.isArray(allowedOperationNames) || allowedOperationNames.length === 0) {
    context.reportError(new _graphql.GraphQLError("Allowed operation names must be specified"));
  }
  return {
    OperationDefinition: function OperationDefinition(node) {
      var operationName = node && node.name && node.name.value;
      if (!allowedOperationNames.includes(operationName)) {
        context.reportError(new _graphql.GraphQLError("Unexpected operation name: " + operationName));
      }
    }
  };
}
